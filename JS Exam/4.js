...
function solve4(arr) {
	...
	for (var i=1; i < arr.length; i++) {
		...
		if (person['role'] == 'trainer') {
			result['trainers'] = result['trainers'] || [];

			var newTrainer = {};
			newTrainer['id'] = person['id'];
			newTrainer['firstname'] = person['firstname'];
			newTrainer['lastname'] = person['lastname'];
			newTrainer['courses'] = person['courses'];
			newTrainer['lecturesPerDay'] = person['lecturesPerDay'];
			...
			newStudent['firstname'] = person['firstname'];
			newStudent['lastname'] = person['lastname'];
			var sum = 0;
			
			person['grades'].forEach(function(grade) {
				sum+=Number(grade);
			});
			
			var length = person['grades'].length;
			newStudent['averageGrade'] = (sum/person['grades'].length).toFixed(2);
			newStudent['certificate'] = person['certificate'];
			newStudent['level'] = person['level'];
	...
	if (firstline.indexOf('level') != -1) {
		result['students'].sort(function (a, b) {
	        if (Number(a['level']) != Number(b['level'])) {
	            return Number(a['level']) - Number(b['level']);
	        } else {
	            return Number(a['id']) == Number(b['id']) ? 0 : Number(a['id']) < Number(b['id']) ? -1 : 1;
	        }
	    });	
	} 
	...
	result['students'].forEach(function (a) {
	        delete a['level'];
	});	
	...
	result['trainers'].sort(function (a, b) {
        if (a['courses'].length != b['courses'].length) {
            ...
            return Number(a['lecturesPerDay']) == Number(b['lecturesPerDay']) ? 0 : Number(a['lecturesPerDay']) < Number(b['lecturesPerDay']) ? -1 : 1;
        }
    });
	
	var newResults = {};
	newResults['students'] = result['students'];
	newResults['trainers'] = result['trainers'];
